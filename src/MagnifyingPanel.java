...
public class MagnifyingPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		int tto = 225;
		int width = 450;
		int x = (MouseLocation.getMouseC()+tto<img.getWidth())? (MouseLocation.getMouseC()-tto>=0)?MouseLocation.getMouseC()-tto:0:img.getWidth()-tto*2;
		int y =(MouseLocation.getMouseR()+tto<img.getHeight())? (MouseLocation.getMouseR()-tto>=0)?MouseLocation.getMouseR()-tto:0:img.getHeight()-tto*2;
		
		//System.out.printf("%d , %d , %d, %d, mouseX:%d mouseY:%d\n",img.getWidth(), img.getHeight(), x,y, MouseLocation.getMouseC(), MouseLocation.getMouseR());
		BufferedImage subimg = img.getSubimage(x,y,width,width);
		
		//		w/4	    (imgwidth - w- (x max - w/2))? x-(x max - w/2): w/4:x
		int xx= (x>112)? (x>img.getWidth()-width-(img.getWidth()-tto*2 -width/2))?x-(img.getWidth()-tto*2 -width/2):width/4:x;
		int yy= (y>112)? (y>img.getHeight()-width-(img.getWidth()-tto*2 -width/2))?y-(img.getHeight()-tto*2 -width/2):width/4:y;
														//y-(y max - w/2
